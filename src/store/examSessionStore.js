...
const useExamSessionStore = create((set, get) => ({
      ...
      if (submissionData.answers) {
        submissionData.answers.forEach((section) => {
          section.questions.forEach((question) => {
            useExamUIStore.getState().setAnswer(
              question.questionId,
  ...
  handleSubmitQuiz: async () => {
    try {
      ...
      const scoreDetails = useExamUIStore.getState().calculateScore(quiz.sections);
  ...
  getAnalytics: () => {
    ...
    if (isReviewMode && submissionData?.analytics) {
      ...
      return {
        ...
        totalScore: storedAnalytics.totalScore || storedAnalytics.score || 0, // Add totalScore field
        sectionWise: storedAnalytics.sectionWise || {},
        rawScore: storedAnalytics.rawScore || storedAnalytics.totalScore || storedAnalytics.score || 0,
      };
    }
      ...
      section.questions.forEach((question) => {
        ...
        if (answers[question.id] !== undefined) {
          ...
          if (answers[question.id] === question.correctAnswer) {
            ...
            analytics.sectionWise[section.name].score += positiveScore;
          } else {
            analytics.incorrect++;
            ...
            analytics.sectionWise[section.name].score -= negativeScore;
          }
        }
      });
    });
